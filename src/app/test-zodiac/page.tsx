...
function TestWheel() {
  const centerX = 250;
  const centerY = 250;
  const houseRingInner = 60;
  const houseRingOuter = 100;
  const zodiacRingInner = 100;
  const zodiacRingOuter = 150;
  const segmentAngle = 30;

  return (
    <svg viewBox="0 0 500 500" style={{ width: '500px', height: '500px' }}>
      {/* Background */}
      <circle cx={centerX} cy={centerY} r={zodiacRingOuter} fill="#0a0e1a" stroke="#00d4ff" strokeWidth="2" />
        ...
        return (
          <g key={i}>
            ...
            <line
              ...
              x2={polarToCartesian(centerX, centerY, zodiacRingOuter, startAngle).x}
              y2={polarToCartesian(centerX, centerY, zodiacRingOuter, startAngle).y}
              stroke="#00d4ff"
              strokeWidth="1"
              opacity="0.6"
            />
            ...
            {/* Zodiac symbol */}
            <g
              transform={`translate(${polarToCartesian(centerX, centerY, (zodiacRingInner + zodiacRingOuter) / 2, midAngle).x - 12}, ${polarToCartesian(centerX, centerY, (zodiacRingInner + zodiacRingOuter) / 2, midAngle).y - 12}) scale(0.24)`}
              style={{ color: '#ffffff' }}
            >
      ...
      {/* Center circle */}
      <circle cx={centerX} cy={centerY} r={houseRingInner} fill="#0a0e1a" stroke="#00d4ff" strokeWidth="1" />

      {/* Ring boundaries */}
      <circle cx={centerX} cy={centerY} r={houseRingOuter} fill="none" stroke="#00d4ff" strokeWidth="1" />
      <circle cx={centerX} cy={centerY} r={zodiacRingOuter} fill="none" stroke="#00d4ff" strokeWidth="2" />
    </svg>
  );
}
...
export default function TestZodiacPage() {
  return (
    <div style={{
      ...
      <h1 style={{ textAlign: 'center', marginBottom: '40px' }}>Zodiac Symbol Test Page</h1>
      ...
      <h2 style={{ textAlign: 'center', marginBottom: '30px' }}>Individual Symbols</h2>
